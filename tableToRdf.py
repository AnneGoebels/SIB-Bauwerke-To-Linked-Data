def tableToRdf ():
    ...
    ont = Graph()
    ont.parse(r"C:\GitHub\TwinGen\Ontologies\ASB_Ontology_FINAL.ttl", format = "turtle")
    ...
    def getAsbClass(name):
        ...
        FILTER(STRSTARTS(STR(?class), "http://asbingowl.org/def/2016/asbingowl#"))}
        """)
            ...
            if "DatatypeProperty" in propType:
                ...
                if "associatedWith" in propRes:
                    ...
                    if x != None:
                        ...
                        if x[3] != None:
                            c.execute("select OBJECT from CURRENT_BRIDGE where FIXVALUE = 'SCHAD_M' and ASBID=?", (asbid))
                            Menge = c.fetchall()
                            if Menge != None:
                                M = Menge[0][0]
                                #print(M)
                                createValueTriple(s, "Schaden_Rissbreite", M, asbid)
                            else:
                                b=BNode()
                                ...
                                createValueTriple(b2, prop3, Menge[0], asbid)
                    else:
                ...
                if fix not in OrtsangabenProcessed:
                    ...
                    # print(rows)
                        ...
                        for t in rows:
                            ps = t[0].split(",")
                        # print(ps)
                            p0 = getAsbProp(ps[0])[0]
                            p1 = ps[1]
                            graph.add((InsObj, p0, b))
                            if len(ps) == 2:
                                #print(t[1])
                                createValueTriple(b, p1, t[1],asbid)
                            else:
                                subject = createBlankTriple(b,p1)
                                #print(t[1])
                                createValueTriple(subject, ps[-1], t[1],asbid)
                    else:
                        pass
